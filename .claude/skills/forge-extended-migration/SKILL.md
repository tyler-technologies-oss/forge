...
## Add a blocks gallery entry

After the component migration itself is verified, invoke the **`forge-blocks-authoring`** skill to create one or more blocks for the newly migrated component, based on its Storybook story/stories (`packages/forge/src/stories/components/<name>/<Name>.stories.ts`) — don't invent a different composition than what the story already documents. This is a normal part of migrating a component out of `forge-extended`, not an optional extra; do it in the same pass unless the user says otherwise.

- If the component is wired in via the subpath-only path (per the "Wiring into `src/lib/index.ts`" section above), remember `blocks/forge-register.ts` needs its own explicit side-effect import (`import '@tylertech/forge/<name>';`) — `defineComponents()` doesn't reach it, so the blocks gallery has to opt in itself, same as any other consumer. This is step 5 in that section; don't skip it just because the component build itself already succeeds.
- If the component is normally demonstrated inside another container (e.g. an app bar, a toolbar) rather than standalone, compose the block that way — see the `app-launcher` block (`blocks/src/blocks/components/app-launcher/demo/`) for a component that's only meaningful wired into a `forge-app-bar`'s `end` slot.
- Follow `forge-blocks-authoring`'s own guidance on wiring real interactivity (a companion `.ts` file) rather than a static, inert markup dump, particularly for anything with array/object properties that can't be expressed as plain HTML attributes.
