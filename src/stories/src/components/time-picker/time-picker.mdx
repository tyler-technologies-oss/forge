...
Gets/sets whether the input mask characters are shown and overwritten as the user types. Only applies if `masked` is `true`. Only shows on focus, hides on blur. Ex. `__:__ __`
