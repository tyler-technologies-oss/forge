import { DefaultTemplate as TimePickerDemo } from './templates/default';
import { TimePickerDefaultHtml } from './code/time-picker-default';
import { TimePickerUnmaskedHtml } from './code/time-picker-unmasked';
...
wraps a [text-field component](?path=/story/components-text-field--default) to provide the Forge look-and-feel, but it can technically attach itself to any
...
while attempting to keep time formats uniform across Tyler applications.
...
Notice that formatting is applied as the user types. Masking provides additional structure and clarity, but can be less forgiving for input that may be incorrect.
...
The component can be configured to not use an input mask (allowing input of any characters), but it will attempt to coerce
the provided time value to a valid format that it supports when the field loses focus.

This option provides less structure up front, but can be configured to be more forgiving of user input.

<LiveDemo codeHtml={TimePickerUnmaskedHtml()}>
  <TimePickerDemo masked={false}/>
</LiveDemo>

> A note about mobile usage, the dropdown list will render within a full screen dialog with larger touch targets when on smaller viewports.
> You can configure the screen size that will trigger this change if desired.
