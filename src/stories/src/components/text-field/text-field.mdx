...
<PropertyDef name="required" type="boolean" defaultValue="false">
  ...
  Controls the presence of an asterisk next to the label to signify that the field is required.
...
- The label element will receive the proper ARIA attributes such as `aria-owns`.

> Verify that the relevant [aria attributes](https://developer.mozilla.org/en-US/docs/Web/Accessibility/ARIA/forms/Basic_form_hints) are set properly when an element is required or invalid.
