...
import { CellAlign, IColumnConfiguration, TextFieldComponentDelegate, ITableFilterEventData, ITableSortEventData, ITableRowClickEventData, ITableComponent } from '@tylertech/forge';
import { ElementProxy, ForgeTable, useForgeToast } from "@tylertech/forge-react";
import React, { MutableRefObject, Ref, RefAttributes, RefObject, useRef, useState } from "react";
import { ITableProps } from "../table-args";
...
export const DefaultTemplate: Story<ITableProps> = ({
  select = false,
  multiselect = true,
  dense = false,
  roomy = false,
  ...
  let displayData = [...people];
  const tableRef = useRef<ITableComponent>();
  ...
  function setData(data: any[]): void {
    displayData = [...data];
    if (tableRef.current) {
      // Setting data manually to avoid React re-rendering the whole component when setting the data property for us
