import { DefaultTemplate as TableDemo } from './templates/default';
...
>
> If your team needs extensive data manipulation or editing capability, **a third-party table extension may be more appropriate.** See our guidance on 
[advanced tables](https://forge.tylertech.com/patterns/external-libraries#advanced-table) under External library guidance. 
...
**Note that the table component requires the use of JavaScript for interaction.** It handles the rendering itself for ease of use, while being flexible in the functionality it provides.
...
Using the component in the HTML markup is simple. Just add the following HTML:

```html
<forge-table></forge-table>
```

This alone won't render anything, but with a little configuration from JavaScript we can setup a table with data easily.

In the example code below, we are getting a reference to the `forge-table` custom element, and then we set the `data` and
`columnConfigurations` properties.

When those two properties are both set, the table will render the rows of data.

<LiveDemo 
fullWidth 
layout={"column"} 
codeHtml={TableDefaultHtml()} 
codeTs={TableDefaultTs()}>
  <TableDemo/>
</LiveDemo>
...
table.columnConfigurations = [
  { property: 'name', header: 'Person name', filter: true, filterDelegate: new TextFieldComponentDelegate({ placeholder: 'Filter name...' }) },
  { property: 'color', header: 'Favorite color', filter: true, filterDelegate: new SelectComponentDelegate({ placeholder: 'Filter color...' }) },
...
```html
<forge-table [columnConfigurations]="columnConfigurations"
           [data]="data$ | async"
           select="true"
           select-key="Id"
...
The following snippet is an example component class with the properties/methods to handle inputs/outputs on the `forge-table` element. The `data` property is being used an RxJS observable here, but by using the `async` pipe we are able to unwrap the observable to provide an array of objects to the table component.
...
| `--mdc-theme-border-color`                              | Controls the border-color of the table.
