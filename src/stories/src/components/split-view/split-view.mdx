...
<PropertyDef name="size" type="number" defaultValue="200">

Sets the initial panel size in pixels. This can also be set to programmatically resize the panel.
...
| Name                                | Description
| :---------------------------------- | ----------------
| `forge-split-view-panel-drag-start` | Emits when the resize handle is grabbed.
| `forge-split-view-panel-drag-end`   | Emits when the resize handle is released.
