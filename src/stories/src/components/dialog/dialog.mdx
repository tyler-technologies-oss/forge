...
It provides a built-in scrim (backdrop), and a surface area for your content. The surface element is sized based on the content you provide to it.

The dialog can be placed in the DOM, and it will not be visible until it is told to open. It contains open/close animations, and provides properties/attributes
for customizing the behavior.

> The dialog component can be created on-the-fly via JavaScript, or used statically within the page and its visibility can be controlled via the `open` property.
...
This example shows a simple dialog this is used statically by controlling the visibility via the `open` property.
...
This example demonstrates how to create a dialog dynamically via JavaScript, and shows how other Forge components can be composed together to create a more complex dialog.
