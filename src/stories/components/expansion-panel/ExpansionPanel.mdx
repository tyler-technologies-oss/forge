...
It is common to place multiple interactive elements within an expansion panel "header" slot. However, you may want to prevent the expansion panel from expanding/collapsing
when a specific element is clicked. To do this you can either listen for the click event yourself on the specific elements you care about and call `stopPropagation()`
on the event, or you can use the `forge-ignore` attribute for convenience. This attribute is used to prevent the expansion panel from expanding/collapsing when the
