...
> **Tip:** Make sure the intended trigger element is present in the DOM at the same time the expansion panel initializes or has `trigger` set. Alternatively, use the `triggerElement` property to directly pass in the trigger element reference.
