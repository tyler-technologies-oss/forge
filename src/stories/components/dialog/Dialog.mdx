...
> Set a width or minimum width on the dialog content to ensure that the dialog is not too narrow and/or wide in various screens sizes.
...
While dialogs are typically created dyanmically and appended to the document, you can also use them inline in your HTML declaratively without any JavaScript via the `trigger` attribute:
