...
Using icons within your Angular application is as simple as using the `<forge-icon>` component. You can import the `ForgeIconModule` from the `@tylertech/forge-angular` package to use the icon component.
...
This is a topic that is often confused when working with custom elements in Angular. When you bind to a property on a custom element, you
are binding to the property of the element itself, not the attribute.

For example, when you bind to the `value` property of a `<forge-slider>` component, you are binding to the property of the component, not
the attribute of the element.

```html
<forge-slider [value]="sliderValue"></forge-slider>
```

This is important to understand because when you bind to a property, you are binding to the component's internal state, not the attribute
of the element. This is why you use square brackets `[]` to bind to properties.

You can however

## CUSTOM_ELEMENTS_SCHEMA

While not recommended, if you prefer not to use the adapter, you can easily disable the Angular compiler from throwing errors by adding the `CUSTOM_ELEMENTS_SCHEMA` to your component or module's `schemas` array:
