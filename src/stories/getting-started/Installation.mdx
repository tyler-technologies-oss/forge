...
- `dist/`: Contains all pre-built static distribution sources.
  - `<component-name>/`: pre-built CSS stylesheets for specific components.
    - Ex. `table/forge-table.css`: Contains the pre-built CSS classes to support the `<forge-table>` component.
  - `esm/`: Contains the bundled and code-split ESM JavaScript sources (includes dependencies). This is for distribution on the Forge CDN, or for self-hosting Forge.
  ...
  - `lib.js`: The pre-built Forge JavaScript library. Contains all Forge components and utilities in a single bundle (including dependencies).
- `esm/`: Contains the unbundled ESM JavaScript sources as the main entry to the package (uses bare module specifiers).
- `styles/`: Contains all Sass stylesheets as a copy directly from the library. This allows you to utilize the same Sass utilities that Forge does within your own application and/or library.
