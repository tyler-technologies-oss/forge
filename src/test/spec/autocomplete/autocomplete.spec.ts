...
describe('AutocompleteComponent', function(this: ITestContext) {
  ...
  describe('with static <input> element', function(this: ITestContext) {
    ...
    it('should toggle linear progress when filtering', async function(this: ITestContext) {
      ...
      expect(linearProgressContainer.classList.contains(LINEAR_PROGRESS_CONSTANTS.classes.CLOSED)).toBe(false);

      await timer(timeout);
      expect(linearProgressContainer.classList.contains(LINEAR_PROGRESS_CONSTANTS.classes.CLOSED)).toBe(true);
    });
