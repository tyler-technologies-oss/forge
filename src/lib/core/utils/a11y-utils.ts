...
/**
 * Options to provide when setting default ARIA properties.
 */
...
export function setDefaultAria(
  ...
  { overwrite }: DefaultARIAOptions = { overwrite: false }
): void {
  ...
  Object.entries(properties).forEach(([key, value]) => {
    ...
    if (value != null) {
      if (overwrite || !element.hasAttribute(ariaAttribute)) {
        toggleAttribute(element, true, ariaAttribute, value.toString());
      }
      storeDefaultAria(element, key as ARIAProperty, value);
    } else {
      element.removeAttribute(ariaAttribute);
      // restoreDefaultAria(element, key as ARIAProperty);
    }
  });
}
...
export function restoreDefaultAria<T extends keyof ARIAMixinStrict>(element: HTMLElement, property: T): void {
  const attributeName = ariaPropertyToAttribute(property);
  const defaultValue = retrieveDefaultAria(element, property);
  if (defaultValue !== null) {
    toggleAttribute(element, true, attributeName, defaultValue.toString());
  } else {
