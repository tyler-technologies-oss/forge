...
- **`aria-controls` referenced an id across a shadow boundary.** The toolbar and all thirteen tool
  ...
  `controlsElement`. It targets the editor rather than the editable element: element references only
  resolve into the same tree or an ancestor tree, and the editable element sits in a sibling branch.
  Feature-detected, so engines without ARIA element reflection simply carry no controls
  relationship.
- **The renderer advertised itself as an unnamed text input.** ProseMirror marks its element
  ...
  The role is kept and declared read-only, which describes it accurately, with the accessible name
  that makes it valid — `role="textbox"` plus `aria-readonly` and no name still fails
