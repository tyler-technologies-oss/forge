...
describe('RichTextEditor accessibility', () => {
  ...
  it('should not reference an id across a shadow boundary', async () => {
    ...
    // aria-controls pointed at an id inside another shadow root, which is never resolvable. The
    // relationship is now an element reference. Chrome reflects that by writing an *empty*
    // aria-controls attribute, so the invariant is that no element carries a non-empty IDREF -
    // an empty value references nothing and axe treats it as valid.
    expect(violations.find(v => v.id === 'aria-valid-attr-value')).toBeUndefined();
    const idrefs = [...el.shadowRoot!.querySelectorAll('[aria-controls]')].map(n => n.getAttribute('aria-controls'));
    expect(idrefs.every(v => v === '')).toBe(true);
  });
});
