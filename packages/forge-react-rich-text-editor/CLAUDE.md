...
```bash
pnpm run --filter @tylertech/forge-react-rich-text-editor build   # regenerate + typecheck
pnpm run --filter @tylertech/forge-react-rich-text-editor test    # Vitest browser mode
pnpm run --filter @tylertech/forge-react-rich-text-editor dev     # demo at localhost:3461
```
