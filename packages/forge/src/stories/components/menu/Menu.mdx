...
Every selection dispatches a single `forge-menu-select` event on the `<forge-menu>` element with the selected option's `value` in the event detail. When a
menu lists actions, that means a handler that branches over every value. Instead, you can attach an `onSelect` callback directly to each option so the
...
The callback receives the same data as the `forge-menu-select` event and runs after the event has been dispatched. Calling `preventDefault()` on the
