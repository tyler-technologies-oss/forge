...
To get the name of the active tab when the active tab changes, listen for the `forge-tab-bar-change` event on the `<forge-tab-bar>` element and access
the `activeTabName` property on the event's target.
