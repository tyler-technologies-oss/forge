...
event on the `<forge-tab-bar>` element and access the `activeTabName` property on the event's
target.

```ts
const tabBar = document.querySelector('forge-tab-bar');
tabBar?.addEventListener('forge-tab-bar-change', (event) => {
  const activeTabName = (event.target as HTMLForgeTabBarElement).activeTabName;
