...
It is common to place multiple interactive elements within a `<forge-list-item>`. However, by default, children of an interactive `<forge-list-item>` will not receive pointer events.
To ensure certain elements receive pointer events instead of the `<forge-list-item>`, you can either listen for the click event yourself on these specific elements and call `stopPropagation()`
