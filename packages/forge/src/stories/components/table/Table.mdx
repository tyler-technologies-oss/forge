...
- `footer` - The text to be displayed in the column footer.
...
## Using column footers

To add a footer row to the table, set the `include-footer` attribute (or `includeFooter` property) to `true` on the `<forge-table>` element. Then configure each column with either a `footer` string or a `footerTemplate` callback to control what each cell in the footer row renders.

Use `footer` for simple static text:
...
Use `footerTemplate` for custom HTML content in footer cells:
...
You can also apply custom styles to footer cells via the `footerCellStyle` property on each column configuration.
