...
When the filter callback returns no results, the default empty state message is displayed to provide feedback to the user. This is useful for indicating that the search returned no matches.

For more control over the empty state content, use the `emptyMessage` property to change it, or the `emptyStateBuilder` callback. The callback receives the current filter text and can return either a `string` or an `HTMLElement`:
...
> **Note:** If both `emptyMessage` and `emptyStateBuilder` are set, the `emptyStateBuilder` takes precedence.
