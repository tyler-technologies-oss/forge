...
const insetFields = document.querySelectorAll(
  'forge-field:where([label-position=inset],:not([label-position])):not([float-label])'
) as NodeListOf<IFieldComponent>;
...
longLabelSwitch.addEventListener('forge-switch-change', () => {
  labels.forEach(
    label =>
      (label.textContent = longLabelSwitch.on
